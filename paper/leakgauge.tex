...
% Three-model evaluation; the result is an honest null (Kendall tau = 1.000).
% ---------------------------------------------------------------------------
...
rankings are \emph{identical} (Kendall $\tau = 1.000$): on these models,
verifying leakage does \emph{not} reorder the ranking. We report this negative
result plainly. The near-floor regime leaves little spread for a reorder to
appear, so the standing contribution is the programmatic, LLM-free
leakage-verification methodology and a reproducible, leakage-verified
leaderboard; surfacing a reorder likely needs harder cases (future work).
\end{abstract}
...
evaluation reported here the two rankings coincide (Kendall $\tau = 1.000$,
...
(Table~\ref{tab:results}): when they act at all, hijack and leakage nearly
coincide, so the per-model gap is small and the ranking does not reorder. This is
why the headline is inherently a \emph{multi-model} claim --- a reorder can only
appear across models that sit at different points of the trade-off between acting
on the injection and selecting the right secret --- and why, in a regime where
every model is near the floor, there is little spread for it to appear.
...
$\tau = 1.000$}. On these models, re-scoring the same traces on verified leakage
does \emph{not} reorder the robustness ranking. GPT-4o's bootstrap 95\% CIs are
$[0.027, 0.146]$ (hijack)
and $[0.027, 0.135]$ (leakage); with $n = 37$ the intervals are wide and overlap
heavily, and the two rates are statistically indistinguishable.
Utility-under-attack is tracked per case but does not bear on the ranking and is
...
and leakage rankings coincide (Kendall $\tau = 1.000$).}
\label{tab:results}
\begin{tabular}{lcccc}
\toprule
Model & \hijackasr & \leakasr & Rank$_{\text{hijack}}$ & Rank$_{\text{leak}}$\\
\midrule
GPT-4o         & $0.081$ & $0.077$ & 1 & 1\\
Llama-3.3-70B  & $0.023$ & $0.018$ & 2 & 2\\
GPT-4o-mini    & $0.005$ & $0.005$ & 3 & 3\\
\midrule
\multicolumn{5}{l}{Kendall $\tau$ (hijack-rank vs.\ leakage-rank) $= 1.000$ --- rankings identical.}\\
\bottomrule
\end{tabular}
\end{table}
...
$\tau = 1.000$. This is the honest null the question returns on current,
near-floor models.}
\label{fig:reorder}
\end{figure}
...
Falsification was explicit, and by that standard our result is a null: the two
rankings agree exactly ($\tau = 1.000$) and \leakasr{} tracks \hijackasr{} case
by case. We report it as such --- a negative result about how injection
benchmarks score, together with the reusable methodology and the leakage-verified
leaderboard --- rather than searching for a framing under which the reorder
survives.
...
reorders. Across three models on $37$ cases the answer is no: the hijack and
leakage rankings coincide (Kendall $\tau = 1.000$), because attack success sits
near the floor and leaves little room for the two to diverge. We report the
negative result plainly. The lasting contribution is the verification methodology
--- leakage decided by canary, never by an LLM judge --- and a reproducible,
leakage-verified leaderboard; whether verified leakage reorders rankings is a
live question that harder cases, not a different scorer, will settle.
