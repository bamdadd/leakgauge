...
\subsection{Case-design principle: injection on the benign critical path}
\label{sec:onpath}

A benchmark of this kind measures a model's \emph{decision} only if the attack is
actually delivered to it. We therefore require that in every case the injected
content sits \emph{on the benign task's critical path}: reading it is a step the
model must take to complete the legitimate request, so an agent that does its job
is guaranteed to encounter the injection, and the protected canary is
\emph{co-located} on that path --- reachable through the tools the benign task
...
\begin{itemize}
  ...
  \item \textbf{Cases required calibration.} An early (v1) case set
        ...
        robustness. This motivated the on-path principle
        (Section~\ref{sec:onpath}); cases were redesigned so injection is on-path
        and the canary co-located. The correction was informed by a single
        inexpensive-model pilot used only to confirm that attacks deliver and
        trigger --- a calibration check, not a tuned or multi-model result; the
        reported evaluation (Section~\ref{sec:results}) uses the redesigned
        cases.
\end{itemize}
